...
class TMTM(nn.Module):
    def __init__(self, hidden_dimension=128, out_dim=2, relation_num=12, dropout=0.3):
        ...
        self.linear_relu_num_prop=nn.Sequential(
            nn.Linear(41, int(hidden_dimension/4)),
            nn.LeakyReLU()
        )
        ...
        # Two separate RGCN layers instead of a single shared layer
    ...
    def forward(self, feature, edge_index, edge_type):
        ...
        # First RGCN layer with residual connection + LayerNorm
        residual = x
        x = self.rgcn1(x, edge_index, edge_type)
        x = self.layer_norm1(x + residual)
        x = F.dropout(x, p=self.dropout, training=self.training)

        # Second RGCN layer with residual connection + LayerNorm
        residual = x
