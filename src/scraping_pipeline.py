...
from tweet_scraper_enhanced import EnhancedTweetScraper
...
class ScrapingPipeline:
    ...
    def run_tweet_scraper(self):
        ...
        NOW WITH ENHANCED USER PROFILE EXTRACTION!
        ...
        try:
            logger.info("")
            logger.info("="*70)
            logger.info("STEP 2: SCRAPING TWEETS FOR HASHTAGS (ENHANCED)")
            logger.info("="*70)
            logger.info("✨ Using Enhanced Scraper with full user profile extraction")
            logger.info("")
            ...
            # Initialize ENHANCED tweet scraper
    ...
    def run(self):
        ...
        logger.info("")
        logger.info("╔"+"═"*68+"╗")
        logger.info("║" + " "*10 + "ENHANCED TWITTER SCRAPING PIPELINE STARTED" + " "*15 + "║")
        logger.info("╚"+"═"*68+"╝")
        logger.info("")
        logger.info(f"Start Time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(f"Target: 5 trending hashtags → 2000 tweets each = 10,000 tweets")
        logger.info(f"✨ Enhanced Mode: Full user profile extraction enabled")
        logger.info(f"📊 Collecting: Tweets + User profiles (followers, bio, etc.)")
        logger.info("")
